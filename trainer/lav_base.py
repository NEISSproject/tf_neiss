...
flags.define_string("debug_dir", "", "specify dir to save debug outputs, saves are task specific ")
...
class LavBase(object):
    ...
    def lav(self):
        ...
        self._task.graph_eval = tf.keras.models.load_model(export_dir_path)
        for (batch, (input_features, targets)) in enumerate(self._input_fn_generator.get_input_fn_val()):
            ...
            loss = tf.keras.losses.mean_absolute_error(self._task.graph_eval._graph_out["pre_points"],
                                                       tf.reshape(targets["points"], (-1, 6)))
            self._task.print_evaluate(self._task.graph_eval._graph_out, targets)
        ...
        self._task.print_evaluate_summary()
