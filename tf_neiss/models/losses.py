...
def map_point3_loss(in_tupel):
    """wrapping point3_loss for use in map"""
    target, prediction = in_tupel
    return point3_loss(target, prediction)
...
def no_match_loss(target, prediction, batch_size):
    ...
    return tf.losses.mean_absolute_error(target_lined, prediction_lined)
...
def map_ordered_point3_loss(in_tupel):
    ...
    return tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(prediction - target), axis=1)))
